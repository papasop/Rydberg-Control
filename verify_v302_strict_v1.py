...
def run_stage(args: list[str], log_fh: Any, timeout_seconds: int) -> dict[str, Any]:
    ...
    try:
        ...
        log(log_fh, f"TIMEOUT WALL_SECONDS {elapsed:.3f}")
        raise VerificationError(f"Subprocess timed out after {timeout_seconds} seconds: {' '.join(args)}") from exc
    elapsed = time.monotonic() - started
...
def fresh_reproduction(args: argparse.Namespace, run_dir: Path, env: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    with log_path.open("w", encoding="utf-8") as log_fh:
        commit = run_stage([sys.executable, str(main_script), "--outdir", str(commit_dir)], log_fh, args.timeout_seconds)
        ...
        validation["source_identity"] = {
            "algorithm": "sha256-file-bytes-v1",
            "external_freeze_source_hash_equals_evaluate_source_hash": True,
            "current_source_hash_equals_expected_source_hash": True,
            "expected_source_file_sha256": expected_source_hash,
            "current_source_file_sha256": sha256_bytes(main_script),
            "historical_manifest_source_hash": manifest_check["historical_manifest_source_hash"],
            "historical_source_identity_status": "HISTORICAL_SOURCE_IDENTITY_REPORTED_NOT_REESTABLISHED",
            "legacy_manifest_source_hash_compared_to_file_bytes": False,
        }
...
def main() -> int:
    ...
    parser.add_argument("--timeout-seconds", type=int, default=1800, help="Timeout for each commit/evaluate subprocess.")
        ...
        certificate = {
            **validation,
            "source_identity": validation.get("source_identity", {
                "current_file_source_identity": env["source_identity"],
                "fresh_freeze_evaluate_pairing": False,
                "historical_source_identity_status": "HISTORICAL_SOURCE_IDENTITY_REPORTED_NOT_REESTABLISHED",
                "legacy_manifest_source_hash_compared_to_file_bytes": False,
            }),
            "execution_success": True,
